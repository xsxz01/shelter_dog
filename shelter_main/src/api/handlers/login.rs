...
pub async fn login(
    ...
    match entity::user::Entity::find()
        ...
        .all(_state.db_conn.load().as_ref())
        .await {
        Ok(admins) => {
            if admins.is_empty() {
    ...
    let secret = &_state.settings.load().token_secret.clone();
    let timeout = _state.settings.load().token_timeout_seconds.clone();
